...
public class AutoPainter {
    ...
    public static class ActionStep {
        ...
        public static ActionStep rightClick() {
            return new ActionStep(Type.RIGHT_CLICK_CANVAS, null, 0, 0, 0);
        }
        ...
        public static ActionStep checkPass() {
            return new ActionStep(Type.CHECK_PASS, null, 0, 0, 0);
        }
    }
    ...
    public void clientTick() {
        ...
        while (!stepQueue.isEmpty()) {
            ...
            if (step.type == ActionStep.Type.CHECK_PASS) {
                stepQueue.poll();
                List<PixelDrawAction> remaining = buildPaintActions();
                if (remaining.isEmpty() || currentPass >= 5) {
                    ...
                    if (client.player != null) {
                        client.player.sendMessage(Text.literal("§aStarting pass " + currentPass + " (" + remaining.size() + " pixels remaining)..."), false);
                    }
                    totalPixelsToDraw = remaining.size();
                    drawnPixelsCount = 0;
                    buildStepQueue(remaining);
                }
                return;
            }

            if (step.type == ActionStep.Type.EQUIP_ITEM) {
                int delay = tryEquipItem(step.itemId);
                if (delay == -1) {
                    // Running out of dye
            ...
            if (step.type == ActionStep.Type.RIGHT_CLICK_CANVAS) {
                // Right-click canvas (interact)
                client.interactionManager.interactEntity(client.player, activeCanvas, net.minecraft.util.Hand.MAIN_HAND);
                drawnPixelsCount++;
                stepQueue.poll();
                tickCounter = Math.max(0, delayTicks);
                return;
    ...
    private int tryEquipItem(String itemId) {
        ...
        if (invSlot >= 0 && invSlot <= 8) {
            if (currentSlot != invSlot) {
                ...
                return 2; // 2 ticks delay for slot change registration
            }
            return 1; // already holding it, 1 tick is enough
        }
        ...
        return 5; // 5 ticks delay for server-side inventory transactions to complete
    }
    ...
    private int findItemInInventory(String itemId) {
        ...
        // Fallback for White Dye and Bone Meal interchangeability
        if (itemId.equals("minecraft:white_dye") || itemId.equals("minecraft:bone_meal")) {
            Identifier fallbackId = Identifier.of(itemId.equals("minecraft:white_dye") ? "minecraft:bone_meal" : "minecraft:white_dye");
            for (int i = 0; i < 36; i++) {
                ItemStack stack = client.player.getInventory().getStack(i);
                if (!stack.isEmpty()) {
                    Identifier itemKey = Registries.ITEM.getId(stack.getItem());
                ...
                if (color != null) {
                    if (currentColors != null) {
                        ...
                        if (pixelIndex >= 0 && pixelIndex < currentColors.length) {
                            ...
                            int colorId = (currentByte & 0xFF) / 4;

                            // Color ID 0 is the empty canvas background.
                            // We must always paint it, so only skip if colorId != 0 and close enough.
                            if (colorId != 0) {
    ...
    private void buildStepQueue(List<PixelDrawAction> actions) {
        ...
        // Wait 20 ticks (1 second) for late server map updates, then run verification pass
        if (autoVerify) {
            stepQueue.add(ActionStep.waitTicks(20));
            stepQueue.add(ActionStep.checkPass());
        }
    }
    ...
    private void onPaintingFinished() {
        if (client.player != null) {
            ...
            if (autoSave && saveName != null && !saveName.trim().isEmpty()) {
                client.player.networkHandler.sendChatCommand("artmap save " + saveName.trim());
            }
        }
    }
}
