...
public class DrawScreen extends Screen {
    ...
    private ButtonWidget dyesOnlyButton;
    private ButtonWidget highlightButton;
    private ButtonWidget autoVerifyButton;
    private boolean autoVerify = false;

    private ButtonWidget autoSaveButton;
    private TextFieldWidget autoSaveTextField;
    ...
    public DrawScreen() {
        super(Text.literal("AutoPalette Panel v1.2.4"));
    }
    ...
    protected void init() {
        ...
        smoothCamButton = addDrawableChild(ButtonWidget.builder(Text.literal("Cam: ON"), button -> {
            smoothCam = !smoothCam;
            updateButtonsText();
        }).dimensions(145, 130, 73, 20).build());

        // Auto-Verify Button
        autoVerifyButton = addDrawableChild(ButtonWidget.builder(Text.literal("Verify: OFF"), button -> {
            autoVerify = !autoVerify;
            updateButtonsText();
        }).dimensions(222, 130, 73, 20).build());
        ...
        // Dyes Only Button
        dyesOnlyButton = addDrawableChild(ButtonWidget.builder(Text.literal("Dyes Only: OFF"), button -> {
            dyesOnly = !dyesOnly;
            updateButtonsText();
            loadSelectedImagePreview();
        }).dimensions(145, 205, 150, 20).build());

        // Highlight Needed Materials Button
        highlightButton = addDrawableChild(ButtonWidget.builder(Text.literal("Highlight: OFF"), button -> {
            com.example.autopalette.client.util.MaterialHighlighter.setActive(!com.example.autopalette.client.util.MaterialHighlighter.isActive());
            if (com.example.autopalette.client.util.MaterialHighlighter.isActive()) {
                com.example.autopalette.client.util.MaterialHighlighter.clear();
                for (MaterialEntry entry : materialsList) {
                    if (entry.itemId != null && !entry.itemId.isEmpty()) {
                        com.example.autopalette.client.util.MaterialHighlighter.addNeededItem(entry.itemId);
                    }
                }
                if (MinecraftClient.getInstance().player != null) {
                    MinecraftClient.getInstance().player.sendMessage(Text.literal("§aMaterial highlighting enabled!"), false);
                }
            } else {
                com.example.autopalette.client.util.MaterialHighlighter.clear();
                if (MinecraftClient.getInstance().player != null) {
                    MinecraftClient.getInstance().player.sendMessage(Text.literal("§cMaterial highlighting disabled."), false);
                }
            }
            updateButtonsText();
        }).dimensions(145, 205, 150, 20).build());
        ...
        // Auto-Save Button
        autoSaveButton = addDrawableChild(ButtonWidget.builder(Text.literal("Save: OFF"), button -> {
            autoSave = !autoSave;
            updateButtonsText();
        }).dimensions(145, 230, 60, 20).build());

        // Auto-Save TextField
        autoSaveTextField = new TextFieldWidget(
                this.textRenderer,
                210,
                230,
                85,
                20,
                Text.literal("Save Name")
        );
        autoSaveTextField.setMaxLength(32);
        autoSaveTextField.setText(saveName);
        addDrawableChild(autoSaveTextField);
        ...
        drawButton = addDrawableChild(ButtonWidget.builder(Text.literal("START DRAWING"), button -> {
            if (AutoPainter.INSTANCE.isActive()) {
                ...
                if (autoSaveTextField != null) {
                    saveName = autoSaveTextField.getText();
                }
        ...
        }).dimensions(145, 255, 150, 20).build());
    ...
    private void updateButtonsText() {
        ...
        smoothCamButton.setMessage(Text.literal("Cam: " + (smoothCam ? "ON" : "OFF")));
        if (autoVerifyButton != null) {
            autoVerifyButton.setMessage(Text.literal("Verify: " + (autoVerify ? "ON" : "OFF")));
        }
        ...
        dyesOnlyButton.setMessage(Text.literal("Dyes Only: " + (dyesOnly ? "ON" : "OFF")));
        if (highlightButton != null) {
            highlightButton.setMessage(Text.literal("Highlight: " + (com.example.autopalette.client.util.MaterialHighlighter.isActive() ? "ON" : "OFF")));
        }
        if (autoSaveButton != null) {
            autoSaveButton.setMessage(Text.literal("Save: " + (autoSave ? "ON" : "OFF")));
        }
    ...
    private ArtMapPalette.MappedColor getClosestColor(int r, int g, int b) {
        ...
        for (ArtMapPalette.MappedColor color : palette.getMappedColors()) {
            if (dyesOnly && color.shade != ArtMapPalette.ShadeLevel.BASE) {
                continue;
            }
            ...
            if (limit == -1) {
                ...
                for (int y = 0; y < 128; y++) {
                    for (int x = 0; x < 128; x++) {
                        ...
                        int g = (rgb >> 8) & 0xFF;
                        int b = rgb & 0xFF;
                        ArtMapPalette.MappedColor closest = getClosestColor(r, g, b);
    ...
    private void calculateMaterialsList() {
        ...
        // Auto-update highlighter if it is active
        if (com.example.autopalette.client.util.MaterialHighlighter.isActive()) {
            com.example.autopalette.client.util.MaterialHighlighter.clear();
            for (MaterialEntry entry : materialsList) {
                if (entry.itemId != null && !entry.itemId.isEmpty()) {
    ...
    private void updateTabVisibility() {
        ...
        if (autoVerifyButton != null) {
            autoVerifyButton.visible = showSettings;
            autoVerifyButton.active = showSettings;
        }
        ...
        dyesOnlyButton.visible = showSettings;
        dyesOnlyButton.active = showSettings;

        if (autoSaveButton != null) {
            autoSaveButton.visible = showSettings;
            autoSaveButton.active = showSettings;
        }
        if (autoSaveTextField != null) {
            autoSaveTextField.visible = showSettings;
            autoSaveTextField.setEditable(showSettings);
        }

        if (highlightButton != null) {
            highlightButton.visible = showMaterialsTab;
            highlightButton.active = showMaterialsTab;
        }
